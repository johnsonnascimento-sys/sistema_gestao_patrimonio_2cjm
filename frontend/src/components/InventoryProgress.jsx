    ...
    return (
        <details className="rounded-2xl border border-slate-200 bg-white p-3 md:p-4 flex flex-col group" open>
            ...
            <div className="mt-3 group-open:block">
                ...
                {!eventoInventarioId ? (
                    <p className="mt-3 text-sm text-slate-600">Nenhum evento ativo.</p>
                ) : progressoQuery.isFetching && !progressoQuery.data ? (
                    <p className="mt-3 text-sm text-slate-600">Carregando progresso...</p>
                ) : progressoQuery.error ? (
                    <p className="mt-3 text-sm text-rose-700">Falha ao carregar progresso.</p>
                ) : (progressoQuery.data || []).length === 0 ? (
                    <p className="mt-3 text-sm text-slate-600">Sem dados de progresso para o momento.</p>
                ) : (
                    <div className="mt-4 flex-1 overflow-auto rounded-lg border border-slate-200 bg-slate-50 p-2 space-y-2 max-h-80">
                            ...
                            return (
                                <div key={idx} className="rounded-xl border border-slate-200 bg-white p-2">
                                    <div className="flex justify-between items-end mb-1">
                                        <p className="text-xs font-semibold text-slate-900">{p.salaEncontrada || 'Desconhecida'}</p>
                                    ...
                                    <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden flex">
                                        <div
                                            className={`h-full ${perc === 100 ? 'bg-emerald-400' : 'bg-violet-500'}`}
                                            style={{ width: `${perc}%` }}
                                        />
                                    </div>
                                </div>
