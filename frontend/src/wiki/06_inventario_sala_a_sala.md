...
Função no sistema: orientar o fluxo de Inventário - Contagem (operação por sala).
-->
...
- `Cadastrar Bens por Sala`: regularização em lote de localização (sem transferir carga).

## Acompanhamento de progresso por sala

Ao acessar **Cadastrar Bens por Sala**, o sistema exibe um card de progresso com:

- **Total**: quantidade total de bens próprios não baixados.
- **Atualizados**: bens com `local_id` vinculado.
- **Pendentes**: bens sem sala atribuída (`local_id IS NULL`).
...
| Após salvar lote na sala | Estatísticas atualizadas automaticamente |
...
- **Concluídos (com sala)**: `local_id IS NOT NULL`

Há paginação de 50 itens e colunas: `Tombamento | Nome | Unidade | Sala`.
