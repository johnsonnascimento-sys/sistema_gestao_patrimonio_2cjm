...
const repoRootCandidate = path.resolve(frontendRoot, "..");
const repoRoot = fs.existsSync(path.resolve(repoRootCandidate, "PROJECT_RULES.md"))
  ? repoRootCandidate
  : frontendRoot;
const wikiDir = path.resolve(__dirname, "..", "src", "wiki");
const outFile = path.resolve(wikiDir, "wikiMeta.generated.js");
const changeLogSourceFile = path.resolve(repoRoot, "docs", "LOG_GERAL_ALTERACOES.md");
const changeLogOutFile = path.resolve(wikiDir, "changeLog.generated.js");
...
function parseMarkdownTableEntries(markdown) {
  ...
  const headerIdx = lines.findIndex((line) =>
    line.toLowerCase().startsWith("| id | datahorautc | usuario | tipo | branch | commit | detalhe | reversaosugerida |")
  );
  ...
  for (let i = headerIdx + 2; i < lines.length; i += 1) {
    ...
    entries.push({
      id: normalizeCell(id),
      dataHoraUTC: normalizeCell(dataHoraUTC),
      usuario: normalizeCell(usuario),
      tipo: normalizeCell(tipo),
      branch: normalizeCell(branch),
      commit: normalizeCell(commit),
      detalhe: normalizeCell(detalhe),
      reversaoSugerida: normalizeCell(reversaoSugerida),
    });
  }
  return entries;
}
...
function generateChangeLogEntries() {
  const generatedAt = new Date().toISOString();
  const sourceExists = fs.existsSync(changeLogSourceFile);

  if (!sourceExists && fs.existsSync(changeLogOutFile)) {
    console.log(
      `[change-log] skip: source not found at ${path.relative(
        repoRoot,
        changeLogSourceFile
      )}; keeping existing ${path.relative(repoRoot, changeLogOutFile)}`
    );
    return;
  }

  const entries = sourceExists
    ? parseMarkdownTableEntries(fs.readFileSync(changeLogSourceFile, "utf8"))
    : [];
  ...
  const body =
    ...
    `export const changeLogEntries = ${JSON.stringify(entries, null, 2)};\n`;

  fs.writeFileSync(changeLogOutFile, body, "utf8");
  console.log(`[change-log] wrote ${path.relative(repoRoot, changeLogOutFile)} at ${generatedAt}`);
}
